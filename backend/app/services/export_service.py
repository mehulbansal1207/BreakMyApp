...
_FOOTER_TEXT = "Generated by BreakMyApp - breakmyapp.tech"
...
def generate_markdown(scan: Any) -> str:
    ...
    # 6e. Custom Vulnerability Scan
    lines.append("### Custom Vulnerability Scan\n")
    custom_exp = cat_exp.get("custom") or ""
    if custom_exp:
        lines.append(custom_exp)
        lines.append("")
    custom_list: list = (findings.get("custom") or {}).get("findings") or []
    if custom_list:
        lines.append("| Rule ID | Severity | File | Line | Message |")
        lines.append("|---------|----------|------|------|---------|")
...
def generate_pdf(scan: Any) -> bytes:
    ...
    pdf.set_font("Helvetica", "B", 11)
    pdf.set_text_color(*_DARK)
    pdf.cell(0, 7, "Custom Vulnerability Scan", ln=True)
    body_text(cat_exp.get("custom") or "")
    custom_list: list = (findings.get("custom") or {}).get("findings") or []
    if custom_list:
        col_w = [40, 22, 55, 15, 48]
        headers = ["Rule ID", "Severity", "File", "Line", "Message"]
